...
const mapFrom = (x: number, min: number, max: number): number =>
  (max - min) * x + min;

/**
 * Maps "x" value in [min..max] range onto [0..1] range linearly
 */
const mapTo = (x: number, min: number, max: number): number =>
  (x - min) / (max - min);
...
export function Knob({
  ...
  const angle = mapFrom(value01, angleMin, angleMax);
  ...
  const changeValue01By = (diff01: number): void => {
    changeValue01To(clamp(value01 + diff01));
  };
      ...
      >
        ...
        >
          <div className='absolute w-full h-full bg-gray-200 rounded-full'>
            ...
            >
              <div className='absolute left-1/2 top-0 h-1/2 w-[2px] -translate-x-1/2 rounded-sm bg-black' />
            </div>
          </div>
        </KnobHeadless>
        <KnobHeadlessOutput
  ...
  return (
    ...
    >
      <input
        ...
        type='number'
        className='w-full border border-gray-50 bg-gray-200 text-center text-gray-50 outline-none [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none'
        onBlur={submit}
        onKeyDown={(event) => {
